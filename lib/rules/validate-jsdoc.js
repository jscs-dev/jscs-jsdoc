    ...
    check: function(file, errors) {
        ...
        Object.keys(scopes).forEach(function(scope) {
            ...
            file.iterateNodesByType(scopes[scope], function(node) {
                // init
                var commentStart = ((node.jsdoc && node.jsdoc.loc) || node.getLoc()).start;
...
function patchNodesInFile(file) {
    ...
    function findFirstNodeInLine(node) {
        ...
        if (!parent || parent.getLoc().start.line !== node.getLoc().start.line) {
            return node;
        }
    ...
    function findDocCommentBeforeNode(node, self) {
        ...
        if (res.getLoc().start.column !== node.getLoc().start.column) {
            return null;
        }
        ...
        if ((self.type === 'FunctionExpression' || self.type === 'ArrowFunctionExpression') &&
            ...
            (self.getLoc().start.line > res.getLoc().end.line + 2)) {
            return null;
        }
